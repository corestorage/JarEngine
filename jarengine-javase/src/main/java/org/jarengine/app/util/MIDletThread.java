...
import org.jarengine.util.ThreadUtils;
...
public class MIDletThread extends Thread {
	...
	private static boolean highPriority = false;
	public static void setHighPriority(boolean value) {
	    highPriority = value;
	    for (Object ctx : midlets.values()) {
	        for (Object t : ((java.util.Map)ctx).keySet()) {
	            if (t instanceof Thread) {
	                ((Thread)t).setPriority(value ? Thread.MAX_PRIORITY : Thread.NORM_PRIORITY);
	            }
	        }
	    }
	}
	public static boolean isHighPriority() { return highPriority; }
	
	public MIDletThread() {
		super(THREAD_NAME_PREFIX + nextThreadNum());
		register(this);
		if (highPriority) setPriority(Thread.MAX_PRIORITY); else setPriority(Thread.NORM_PRIORITY);
	}
