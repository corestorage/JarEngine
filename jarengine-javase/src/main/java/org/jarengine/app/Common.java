...
import org.jarengine.Injected;
import org.jarengine.MIDletAccess;
import org.jarengine.MIDletBridge;
import org.jarengine.MIDletContext;
import org.jarengine.MIDletEntry;
import org.jarengine.MicroEmulator;
import org.jarengine.RecordStoreManager;
import org.jarengine.app.classloader.ExtensionsClassLoader;
import org.jarengine.app.classloader.MIDletClassLoader;
import org.jarengine.app.classloader.MIDletClassLoaderConfig;
import org.jarengine.app.launcher.Launcher;
import org.jarengine.app.ui.Message;
import org.jarengine.app.ui.ResponseInterfaceListener;
import org.jarengine.app.ui.StatusBarListener;
...
import org.jarengine.app.util.MIDletResourceLoader;
import org.jarengine.app.util.MIDletSystemProperties;
import org.jarengine.app.util.MIDletThread;
import org.jarengine.app.util.MIDletTimer;
import org.jarengine.app.util.MIDletTimerTask;
import org.jarengine.app.util.MidletURLReference;
...
import org.jarengine.log.StdOutAppender;
import org.jarengine.microedition.ImplFactory;
import org.jarengine.microedition.ImplementationInitialization;
import org.jarengine.microedition.io.ConnectorImpl;
import org.jarengine.util.Base64Coder;
import org.jarengine.util.JadMidletEntry;
import org.jarengine.util.JadProperties;
import org.jarengine.util.MemoryRecordStoreManager;
...
public class Common implements MicroEmulator, CommonInterface {
    ...
    public boolean initParams(List params, DeviceEntry defaultDevice, Class defaultDeviceClass) {
        // DEBUG: Check if device.xml is found in classpath
        try {
            InputStream is = getClass().getResourceAsStream("/org/jarengine/device/resizable/device.xml");
            System.out.println("[DEBUG] Device XML found in classpath? " + (is != null));
            if (is != null) is.close();
        } catch (Exception e) {
            System.out.println("[DEBUG] Exception checking device.xml: " + e);
        }
        ...
        if (deviceDescriptorLocation != null) {
            try {
                DeviceImpl device = DeviceImpl.create(emulatorContext, classLoader, deviceDescriptorLocation, defaultDeviceClass);
        ...
        if (DeviceFactory.getDevice() == null) {
            try {
                if (deviceClass == null) {
                    ...
                    DeviceImpl device = DeviceImpl.create(emulatorContext, classLoader, defaultDevice.getDescriptorLocation(), defaultDeviceClass);
                    device.init(emulatorContext);
                    setDevice(device);
    ...
    private MIDletClassLoader createMIDletClassLoader(boolean forJad) {
        ...
        if (!Serializable.class.isAssignableFrom(Injected.class)) {
            Logger.error("classpath configuration error, Wrong Injected class detected. jarengine-injected module should be after jarengine-javase in eclipse");
        }
