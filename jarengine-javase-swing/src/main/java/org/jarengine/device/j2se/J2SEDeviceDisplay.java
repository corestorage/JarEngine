...
import org.jarengine.device.impl.Button;
import org.jarengine.device.impl.Color;
import org.jarengine.device.impl.DeviceDisplayImpl;
import org.jarengine.device.impl.PositionedImage;
import org.jarengine.device.impl.Rectangle;
import org.jarengine.device.impl.Shape;
...
public class J2SEDeviceDisplay implements DeviceDisplayImpl 
{
	...
	public void paintControls(Graphics g) {
		...
		if (g == null || backgroundColor == null || displayRectangle == null || displayPaintable == null || device == null) {
			System.err.println("[JarEngine] paintControls: One or more required fields are null. Skipping paint.");
			return;
		}
		...
		g.setColor(foregroundColor != null ? foregroundColor : java.awt.Color.BLACK);
		if (device.getSoftButtons() != null) {
			for (Enumeration s = device.getSoftButtons().elements(); s.hasMoreElements();) {
				Object btn = s.nextElement();
				if (btn instanceof J2SESoftButton) {
					((J2SESoftButton) btn).paint(g);
				}
			}
		}

		int inputMode = device.getInputMethod() != null ? device.getInputMethod().getInputMode() : -1;
		try {
			if (inputMode == InputMethod.INPUT_123 && mode123Image != null && mode123Image.getImage() != null && mode123Image.getRectangle() != null) {
			...
			} else if (inputMode == InputMethod.INPUT_ABC_UPPER && modeAbcUpperImage != null && modeAbcUpperImage.getImage() != null && modeAbcUpperImage.getRectangle() != null) {
			...
			} else if (inputMode == InputMethod.INPUT_ABC_LOWER && modeAbcLowerImage != null && modeAbcLowerImage.getImage() != null && modeAbcLowerImage.getRectangle() != null) {
			...
			System.err.println("[JarEngine] paintControls: Exception drawing input mode image: " + ex);
		}
	}
	...
	public Button createButton(int skinVersion, String name, org.jarengine.device.impl.Shape shape, int keyCode,
