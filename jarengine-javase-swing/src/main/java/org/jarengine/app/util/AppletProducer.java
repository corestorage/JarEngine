...
import org.jarengine.app.classloader.ClassPreprocessor;
import org.jarengine.app.classloader.ExtensionsClassLoader;
import org.jarengine.app.classloader.InstrumentationConfig;
...
public class AppletProducer {
	...
	public static void createHtml(File htmlOutputFile, DeviceImpl device, String className, File midletOutputFile,
		...
		try {
			...
			writer.write("\t</head>\n");
			writer.write("\t<body>\n");
			writer.write("\t\t<applet code=\"org.jarengine.applet.Main\"\n");
	...
	public static void createMidlet(URL midletInputUrl, File midletOutputFile) throws IOException {
		...
		try {
			...
			URL url = AppletProducer.class.getResource("/jarengine-injected.jar");
			if (url != null) {
				ijis = new JarInputStream(url.openStream());
				while ((jarEntry = ijis.getNextJarEntry()) != null) {
					if (jarEntry.getName().equals("org/jarengine/Injected.class")) {
				...
				Logger.error("Cannot find jarengine-injected.jar resource in classpath");
			}
		} finally {
	...
	private static String usage() {
		return "--midletClass {midlet class name} \n" +
			   "--appletInput {jarengine-javase-applet.jar location} \n" +
