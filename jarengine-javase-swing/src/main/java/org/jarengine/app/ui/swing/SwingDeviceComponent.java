...
import org.jarengine.device.j2se.J2SEDeviceButtonsHelper;
import org.jarengine.device.j2se.J2SEDeviceDisplay;
import org.jarengine.device.j2se.J2SEImmutableImage;
import org.jarengine.device.j2se.J2SEInputMethod;
import org.jarengine.device.j2se.J2SEMutableImage;
...
public class SwingDeviceComponent extends JPanel implements KeyListener, InputMethodListener, InputMethodRequests {
	...
	private MouseAdapter mouseListener = new MouseAdapter() {
		...
		public void mousePressed(MouseEvent e) {
			...
			if (pressedButton != null) {
				...
				org.jarengine.device.impl.Rectangle r = pressedButton.getShape().getBounds();
		...
		public void mouseReleased(MouseEvent e) {
			...
			// optimize for some video cards.
			if (prevOverButton != null) {
				org.jarengine.device.impl.Rectangle r = prevOverButton.getShape().getBounds();
				smartRepaint(new java.awt.Rectangle(r.x, r.y, r.width, r.height));
			} else {
				smartRepaint();
			}
	...
	private MouseMotionListener mouseMotionListener = new MouseMotionListener() {
		...
		public void mouseMoved(MouseEvent e) {
			...
			if (overButton != prevOverButton) {
				...
				if (overButton != null) {
					org.jarengine.device.impl.Rectangle r = overButton.getShape().getBounds();
					smartRepaint(new java.awt.Rectangle(r.x, r.y, r.width, r.height));
				}
	...
	public void keyPressed(KeyEvent ev) {
		...
		if (button != null) {
			...
			org.jarengine.device.impl.Shape shape = button.getShape();
			if (shape != null) {
				org.jarengine.device.impl.Rectangle r = shape.getBounds();
				smartRepaint(new java.awt.Rectangle(r.x, r.y, r.width, r.height));
			}
		} else {
		}
	...
	public void keyReleased(KeyEvent ev) {
		...
		pressedButton = null;
		if (prevOverButton != null) {
			org.jarengine.device.impl.Shape shape = prevOverButton.getShape();
			if (shape != null) {
				org.jarengine.device.impl.Rectangle r = shape.getBounds();
				smartRepaint(new java.awt.Rectangle(r.x, r.y, r.width, r.height));
			}
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (overButton != null) {
            org.jarengine.device.impl.Shape shape = overButton.getShape();
            if (shape != null) {
        ...
        if (pressedButton != null) {
            org.jarengine.device.impl.Shape shape = pressedButton.getShape();
            if (shape != null) {
	...
	private void drawImageInShape(Graphics g, Image image, org.jarengine.device.impl.Shape shape) {
		Shape clipSave = g.getClip();
		if (shape instanceof org.jarengine.device.impl.Polygon) {
			Polygon poly = new Polygon(((org.jarengine.device.impl.Polygon) shape).xpoints,
					((org.jarengine.device.impl.Polygon) shape).ypoints,
					((org.jarengine.device.impl.Polygon) shape).npoints);
			g.setClip(poly);
		}
		// Always use org.jarengine.device.impl.Rectangle for device logic
		org.jarengine.device.impl.Rectangle r = shape.getBounds();
