...
package org.jarengine.app.launcher;
...
import org.jarengine.app.CommonInterface;
...
public class Launcher extends MIDlet implements CommandListener {
	...
	protected static final String NOJARS = "No Jars found. Please load a Jar.";
	...
	public MIDletEntry getSelectedMidletEntry() {
		if (menuList != null) {
			...
			if (!menuList.getString(idx).equals(NOJARS)) {
	...
	public void startApp() {
		...
		if (midletEntries.size() == 0) {
			// Clean, text-only placeholder
			menuList.append(NOJARS, null);
		} else {
